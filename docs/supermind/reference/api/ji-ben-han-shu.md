...
| :------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- | -------- | -------- | -------- | -------- | ---- | ------- |
| [init](/supermind/reference/api/ji-ben-han-shu#初始化函数-init)                                                       | ✅   | ✅       | ✅       | ✅       | ✅       | ✅   | ✅      |
| [handle_bar](/supermind/reference/api/ji-ben-han-shu#按交易频率调用函数-handle-bar)       | ✅   | ✅       | ✅       | ✅       | ✅       | ✅   | ✅      |
| [handle_tick](/supermind/reference/api/ji-ben-han-shu#tick行情数据变化时调用-handle-tick) | ✅   | ✅       | ❌       | ❌       | ❌       | ❌   | ❌      |
| [open_auction](/supermind/reference/api/ji-ben-han-shu#集合竞价后-9-26-调用-open-auction)               | ✅   | ❌       | ❌       | ❌       | ❌       | ❌   | ❌      |
| [before_trading](/supermind/reference/api/ji-ben-han-shu#开盘前半小时调用-before-trading)        | ✅   | ✅       | ✅       | ✅       | ✅       | ✅   | ✅      |
| [after_trading](/supermind/reference/api/ji-ben-han-shu#收盘后半小时调用-after-trading)          | ✅   | ✅       | ✅       | ✅       | ✅       | ✅   | ✅      |
| [on_order](/supermind/reference/api/ji-ben-han-shu#委托回调函数-on-order)                                      | ✅   | ❌       | ❌       | ❌       | ❌       | ❌   | ❌      |
| [on_trade](/supermind/reference/api/ji-ben-han-shu#成交回调函数-on-trade)                                      | ✅   | ❌       | ❌       | ❌       | ❌       | ❌   | ❌      |
...
- 📚参数说明
  - `context`: [context对象](/supermind/reference/api/zhong-yao-dui-xiang#context)，用于存放当前账户资金、持仓信息等数据
- ❗注意事项
...
- 📚参数说明
  - `context`: [context对象](/supermind/reference/api/zhong-yao-dui-xiang#context)，用来存放当前账户资金、持仓信息等数据。
  - `bar_dict`: [bar_dict对象](/supermind/reference/api/zhong-yao-dui-xiang#bar-dict)，用于存放当前订阅所有合约的bar行情数据。
- ❗注意事项
...
- 📚参数说明
  - `context`: [context对象](/supermind/reference/api/zhong-yao-dui-xiang#context)，用来存放当前账户资金、持仓信息等数据。
  - `odr`: [`Order`对象](/supermind/reference/api/zhong-yao-dui-xiang#订单对象-order类)，包含委托详情。
- ❗注意事项
...
- 📚参数说明
  - `context`: [context对象](/supermind/reference/api/zhong-yao-dui-xiang#context)，用来存放当前账户资金、持仓信息等数据。
  - `trade`: [`Trade`对象](/supermind/reference/api/zhong-yao-dui-xiang#成交对象-trade类)，包含成交详情。
- ❗注意事项
  - 回测时下单后立刻触发，仿真交易时会在handle_bar执行完后触发。
- 📝示例：
  - 调用
