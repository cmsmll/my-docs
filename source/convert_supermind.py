...
站内链接重写，并产出 supermind 文档集的侧边栏 sidebar-supermind.json；
导航由 docs/.vitepress/docs-registry.ts 统一生成（全站唯一真源），本脚本不写 nav。
...
标题层级：页面标题写入 frontmatter 并在正文首行以 `#` 呈现（与 iFinD 文档集一致），
页内其余标题整体上移一级，保证层级连续。

输出位置：docs/supermind/（URL 前缀 /supermind/），见 COLLECTION 常量。

锚点：与 VitePress（@mdit-vue/shared）的 slugify 逐字符等价，并按页内重复标题
追加 -1/-2 后缀；`#锚点` 与 `/view/help/N#锚点` 链接统一改写为站点内路径。

用法：python source/convert_supermind.py
"""
...
# 本脚本输出到单站点的 SuperMind 文档集目录下：docs/supermind/
# URL 前缀与目录名一致（/supermind/...），见 docs/.vitepress/docs-registry.ts。
COLLECTION = 'supermind'
...
def fetch(source_dir: Path) -> str:
    ...
    cached = source_dir / 'supermind-help-8.html'  # 与归档素材同名，避免重复下载
    if cached.exists():
    ...
    for doc_title, doc in docs.items():
        ...
        for title, body, level in plan_pages(doc['content']):
            ...
            pages.append({'dir': out_dir, 'file': slug, 'title': name,
                          ...
                          'doc': doc_title,
                          'path': f'/{COLLECTION}/{out_dir}/{slug}'})
...
def main():
    ...
    source_dir = Path(__file__).resolve().parent        # 仓库根/source（原始素材与脚本同处）
    docs_dir = root / 'docs' / COLLECTION               # 仓库根/docs/supermind（本文档集内容）
    ...
    _write_sidebar(planned, docs_dir, written)

    sizes = sorted(len(b) for _, b in written)
    print(f'单页正文 最小 {sizes[0]} / 中位 {sizes[len(sizes) // 2]} / 最大 {sizes[-1]} 字符')


def _write_sidebar(planned, docs_dir, written):
    """生成 sidebar-supermind.json（SuperMind 文档集的侧边栏）。

    侧边栏：@vue/theme 的侧边栏只有**两级**（分组标题 + 平铺链接，VPSidebarGroup
    只渲染 items 里的 link，不支持再嵌套），所以四个子分组以「同一目录键下的四个
    分组」呈现——进入 other 下任一面都能看到这四组，组标题即子分组名。

    只产出侧边栏：导航由 docs/.vitepress/docs-registry.ts 统一生成（全站唯一真源），
    本脚本不再写 nav.json。
    """
    children = {}
    ...
    # 合并目录：四个子分组同挂在一个目录键下（键含文档集前缀）
    sidebar = {
        f'/{COLLECTION}/{MERGED_DIR}/': [{'text': t, 'items': items_of(by_doc[d])}
                                         for d, t in SIDEBAR_GROUPS]
    }
    for doc_title, out_dir, pages in planned:
        if doc_title in merged:
            continue
        sidebar[f'/{COLLECTION}/{out_dir}/'] = [{'text': doc_title, 'items': items_of(pages)}]

    # 侧边栏是**站点配置**，必须写到 docs/.vitepress/（与 config.mts 同级），
    # 而不是文档集目录下——config.mts 从这里 import。
    cfg_dir = docs_dir.parent / '.vitepress'
    cfg_dir.mkdir(parents=True, exist_ok=True)
    (cfg_dir / 'sidebar-supermind.json').write_text(
        json.dumps(sidebar, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
    print(f'侧边栏 {len(sidebar)} 个目录键 / {len(written)} 页')
