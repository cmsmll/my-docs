...
- 🔢返回值说明
  ...
  - `Query`，在[`run_query`](/reference/api/biao-shu-ju-jie-kou#取表数据-run-query)和[`get_fundamentals`](/reference/api/biao-shu-ju-jie-kou#取财务表数据-get-fundamentals)中使用
- 🚀️使用方法：
...
- 📚参数说明：
  ...
  - `query_object`: `Query`，构造方法可参考[query](/reference/api/biao-shu-ju-jie-kou#构建数据表查询对象-query)
- 🔢返回值说明
