...
- ⛺数据结构
  ...
  | 字段名      | 含义                                                                                                             |
  | ----------- | ---------------------------------------------------------------------------------------------------------------- |
  | `run_info`  | 策略运行的配置信息（起止时间、初始资金、频率等），见[run_info](/reference/api/zhong-yao-dui-xiang#run-info) |
  | `portfolio` | 当前投资组合总览，包含股票和期货账户及持仓汇总，见[portfolio](/reference/api/zhong-yao-dui-xiang#portfolio) |
- ❗注意事项：
...
| `positions`       | 类似`dict`          | 持仓情况，key为标的代码，value为[StockPosition](/reference/api/zhong-yao-dui-xiang#stockposition类)对象或[FuturePosition](/reference/api/zhong-yao-dui-xiang#futureposition类)对象 |
| `datetime`        | `datetime.datetime` | 当前行情时间戳                                                                                                                                                                                             |
| `stock_account`   | `StockAccount`      | 股票子账户详情，**期货回测中为`None`**，见[`stock_account`](/reference/api/zhong-yao-dui-xiang#stock-account)                                                                                         |
...
| `positions`      | 类似`dict` | 持仓情况，key为标的代码，value为[StockPosition](/reference/api/zhong-yao-dui-xiang#stockposition类)对象 |
| `pnl`            | `float`    | 股票账户累计盈亏                                                                                                    |
...
| `positions`        | 类似`dict` | 持仓情况，key为标的代码，value为[`FuturePosition`](/reference/api/zhong-yao-dui-xiang#futureposition类)对象 |
| `margin`           | `float`    | 总占用保证金                                                                                                            |
...
- ⛺数据结构:
  ...
  - 类似`dict`，key为标的代码，value为[`Bar`](/reference/api/zhong-yao-dui-xiang#bar类)对象
- ❗注意事项：
...
| `type`             | `ORDER_TYPE`        | 委托类型，见[`ORDER_TYPE`](/reference/api/mei-ju-chang-liang#order-type-委托类型)     |
| `limit_price`      | `float`             | 委托价格，市价单为`0`                                                                                                     |
| `amount`           | `int`               | 委托数量                                                                                                                  |
| `filled_amount`    | `int`               | 已成交数量                                                                                                                |
| `avg_price`        | `float`             | 成交均价，未成交时为`0`                                                                                                   |
| `transaction_cost` | `float`             | 手续费，未成交时为`0`                                                                                                     |
| `status`           | `ORDER_STATUS`      | 订单状态，见[`ORDER_STATUS`](/reference/api/mei-ju-chang-liang#order-status-委托状态) |
...
| `side`               | `SIDE`                      | 交易方向，见[`SIDE`](/reference/api/mei-ju-chang-liang#side-交易方向)                                                    |
| `commission`         | `float`                     | 手续费                                                                                                                                                      |
| `tax`                | `float`                     | 税费                                                                                                                                                        |
| `transaction_cost`   | `float`                     | 总交易成本 =`commission + tax`                                                                                                                              |
| `position_effect`    | `POSITION_EFFECT` 或 `None` | 仓位类型，**期货有效**，股票为`None`，见[`POSITION_EFFECT`](/reference/api/mei-ju-chang-liang#position-effect-仓位类型) |
